...
import {
  assignCategoryForLeagueAction,
  createPlayerForLeagueAction,
  enrollLeagueTeamAction,
  removeLeagueTeamAction,
  searchPlayersForLeagueAction,
} from "../application/leagueEnrollmentActions";
...
function SelectedPlayerChip({
  ...
  function handleAssign() {
    ...
    startTransition(async () => {
      const result = await assignCategoryForLeagueAction(leagueId, player.playerId, Number(category));
...
function AddTeamForm({
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      startTransition(async () => {
        const result = await searchPlayersForLeagueAction(leagueId, query);
  ...
  function handleEnroll() {
    ...
    startTransition(async () => {
      const [a, b] = selected;
      const result = await enrollLeagueTeamAction(leagueId, categoryId, categoryGender, [a.playerId, b.playerId]);
...
function CreatePlayerInlineForm({
  ...
  function handleCreate() {
    setError(null);
    startTransition(async () => {
      const result = await createPlayerForLeagueAction(leagueId, firstName, lastName, gender, Number(category) || 0, phone, email);
...
export function LeagueEnrollmentPanel({
  leagueId,
  categoryId,
  categoryGender,
  teams: initialTeams,
  ...
  function handleRemove(teamId: string) {
    startTransition(async () => {
      const result = await removeLeagueTeamAction(leagueId, teamId);
