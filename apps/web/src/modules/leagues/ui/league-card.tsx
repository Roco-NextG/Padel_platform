...
import { leagueCardStatus, LEAGUE_CARD_STATUS_LABELS, type League } from "../domain/league";
      ...
      <Card interactive className="relative flex h-full flex-col gap-3 overflow-hidden !p-0">
        ...
        <div className="relative z-[1] flex flex-1 flex-col gap-3 px-4 pb-4">
          <div className="flex flex-col gap-1">
            <h3 className="truncate font-display text-sm font-semibold tracking-tight">{league.name}</h3>
            <p className="text-xs text-muted-foreground">{formatDateRange(league.startDate, league.endDate)}</p>
          </div>
          <div className="mt-auto flex items-center justify-between gap-2 border-t border-border pt-3">
            <div className="flex items-center gap-4 text-xs text-muted-foreground">
              <span className="flex items-center gap-1.5">
                <SquaresFour className="size-3.5" />
                {league.categoryCount} {league.categoryCount === 1 ? "categoría" : "categorías"}
              </span>
              <span className="flex items-center gap-1.5">
                <UsersThree className="size-3.5" />
                {league.teamCount} {league.teamCount === 1 ? "pareja" : "parejas"}
              </span>
            </div>
            <Link
              href={editPath}
...
export function NewLeagueCard() {
  return (
    <motion.div whileHover={{ y: -3 }} transition={{ type: "spring", stiffness: 400, damping: 28 }}>
      <Link href="/dashboard/ligas/nuevo">
        <Card dashed interactive className="flex h-full min-h-[170px] flex-col items-center justify-center gap-2 text-center">
          <Plus className="size-5 text-muted-foreground" />
          <span className="text-sm font-medium text-muted-foreground">Nueva liga</span>
        </Card>
      </Link>
    </motion.div>
  );
